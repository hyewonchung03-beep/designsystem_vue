<!--
  ...
  ── 콜백 prop → emit 변환 ────────────────────────────────────────────────────
  이 코드베이스 컨벤션(DataToolbar.vue의 onPerPageChange→perPageChange 등)을 따라
  "on" 접두어를 떼고 camelCase emit으로 변환했다:
  onStoreChange→storeChange, onBack→back, onForward→forward, onClose→close, onSave→save.

  ── Select 연동 ──────────────────────────────────────────────────────────────
  React 원본은 `value={storeName || undefined}` 로 Select에 값을 넘겼다(storeName이
  빈 문자열이면 Select 자체의 비제어 fallback이 동작하도록). Vue Select.vue는
  modelValue/update:modelValue 컨벤션을 쓰므로 동일하게
  `:model-value="storeName || undefined"` + `@update:model-value="emit('storeChange', $event)"`
  로 연결했다 — Select.vue를 재구현하지 않고 그대로 사용.

  ── Globe/Bell/Back/Forward 아이콘 ───────────────────────────────────────────
  React 원본은 이 4개를 위해 파일 로컬 `IconTrigger` 서브컴포넌트(하버 시 원형
  배경이 나타나는 버튼, inset -4px)를 직접 구현했다. 이는 CLAUDE.md의 ic_trigger
  카테고리(아이콘 자체가 독립 인터랙션을 가지는 경우)와 정확히 일치하고, 이 Vue
  코드베이스에는 동일한 패턴의 공용 컴포넌트 `Icon/IcTrigger.vue`가 이미 포팅되어
  있다(overlay `-inset-1 rounded-circle` + `bg-interaction-neutral-normal
  group-hover:...-hover group-active:...-pressed`, size 12/14/16/20/24 지원 — 원본의
  24px/20px 요구사항을 그대로 충족). 로컬 서브컴포넌트를 새로 만들지 않고 기존
  IcTrigger.vue를 재사용했다. 차이점: (1) IcTrigger는 `ariaLabel`이 필수라 각 버튼에
  의미있는 라벨을 부여했고(원본엔 없던 접근성 개선), (2) IcTrigger는 focus-visible 시
  링을 보여준다(원본엔 없음). IcTrigger의 hover transition-colors duration-150은
  이후 IcTrigger.vue 자체에 추가되어 원본과 동일해졌다. 시각적 구조(hover 원형 배경,
  아이콘 크기/색상)는 동일하다.
-->
<script lang="ts">
...
import BadgeStatus from '../Badge/BadgeStatus.vue';
...
<template>
  <div :class="`bg-surface-default text-element-primary text-left flex flex-col w-full ${$props.class}`">
    <div class="flex items-center justify-end gap-5 h-16 px-6">
        ...
        <div class="flex-1 flex items-center justify-end gap-5 min-w-0">
          ...
          <div class="relative shrink-0">
            ...
            <div
              ...
              class="absolute top-0 left-full -translate-x-1/2 -translate-y-1/2 pointer-events-none"
            >
          ...
          <!-- Profile avatar -->
          <div class="shrink-0 w-8 h-8 rounded-full overflow-hidden bg-fill-subtle flex items-center justify-center">
            <img v-if="profileSrc" :src="profileSrc" alt="profile" class="w-full h-full object-cover" />
            <svg v-else width="20" height="20" viewBox="0 0 24 24" fill="none" aria-hidden="true" class="text-element-quaternary">
      ...
      <template v-else-if="type === 'edit'">
        <!-- Breadcrumb -->
        <p class="flex-1 min-w-0 text-text-sm leading-text-sm text-element-tertiary truncate text-left">
          {{ storeName }}
        </p>

        <!-- Right section -->
        <div class="flex gap-2 items-center shrink-0">
          <div v-if="showNavigator" class="flex items-center">
            <!-- Back icon_trigger -->
            <div class="px-4 py-1">
              <IcTrigger :size="20" aria-label="Back" @click="emit('back')">
            ...
            <div class="w-px h-4 bg-border-solid-variant shrink-0" />
            <!-- Forward icon_trigger -->
            <div class="px-4 py-1">
              <IcTrigger :size="20" aria-label="Forward" @click="emit('forward')">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" aria-hidden="true">
          ...
          <span v-if="showText" class="text-text-xs leading-text-xs text-element-tertiary whitespace-nowrap">
            {{ text }}
          </span>
