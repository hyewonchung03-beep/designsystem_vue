<!--
  DatatableTree Pagination — DatatableTree 전용 내부 페이지네이션 (non-exported, private)

  React: src/components/DatatableTree/DatatableTree.tsx (Pagination, 파일 내부 전용 함수)
  DataTable/DataTablePagination.vue와 클래스/아이콘/알고리즘이 사실상 동일하지만
  React 원본 자체가 두 파일에 각각 독립적으로 중복 구현되어 있어(공유 컴포넌트로
  추출돼 있지 않음) 이식 시에도 동일하게 중복 포팅했다 — 원본 구조를 그대로
  유지하기 위함이며 최상위 Pagination/Pagination.vue와도 무관한 별개 구현이다.
-->
...
function getVisiblePages(current: number, total: number): number[] {
  ...
  if (start < 1) { start = 1; end = Math.min(maxVisible, total); }
  if (end > total) { end = total; start = Math.max(1, total - maxVisible + 1); }
  ...
  <nav aria-label="Pagination" class="flex items-center justify-center gap-3 text-left">
    <div class="flex items-start">
      <button
        ...
        class="flex size-9 items-center justify-center text-element-primary disabled:text-element-disabled hover:bg-interaction-neutral-hover active:bg-interaction-neutral-pressed rounded-circle focus-visible:outline-none focus-visible:shadow-focus-button-ring"
        @click="emit('change', 1)"
      >
    ...
    <div class="flex items-center gap-2">
      <input
        ...
        class="h-[30px] w-[34px] rounded-4 border border-border-normal bg-surface-container px-2.5 py-2 text-center text-text-xs leading-text-xs font-regular text-element-quaternary outline-none focus:shadow-[0_0_0_2px_var(--color-primary)]"
        @keydown="handleInputSubmit"
      />
      <span class="text-text-sm leading-text-sm font-regular text-element-quaternary">of {{ totalPages }} page</span>
    </div>
  </nav>
</template>
