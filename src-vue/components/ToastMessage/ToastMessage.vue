...
// ── Status icon (ic_state: 상태에 1:1 매핑되어 소비자가 교체 불가) ────────────
const STATUS_ICON: Record<Exclude<ToastMessageStatus, 'default'>, StatusIconConfig> = {
  success: {
...
defineEmits<{ close: [MouseEvent]; action: [MouseEvent] }>();

const titleCls = computed(() => (props.inverse ? 'text-element-inverse' : 'text-element-primary'));
const descCls = computed(() => (props.inverse ? 'text-element-inverse-variant' : 'text-element-tertiary'));
const actionCls = computed(() => (props.inverse ? 'text-element-inverse' : 'text-element-brand-variant'));
const closeCls = computed(() => (props.inverse ? 'text-element-inverse' : 'text-element-quaternary'));
const bgCls = computed(() => (props.inverse ? 'bg-surface-inverse-surface' : 'bg-surface-container'));
const borderCls = computed(() => (props.inverse ? '' : 'border border-border-solid'));

const actionOverlay = computed(() =>
  props.inverse
...
const closeOverlay = computed(() =>
  props.inverse
    ? 'bg-interaction-inverse-normal group-hover:bg-interaction-inverse-hover group-active:bg-interaction-inverse-pressed'
    : 'bg-interaction-neutral-normal group-hover:bg-interaction-neutral-hover group-active:bg-interaction-neutral-pressed',
);

const hasBody = computed(() => props.showDescription || props.showAction);

const rootClass = computed(() => [
  'inline-flex gap-3 items-start overflow-hidden px-3 py-2 rounded-4 shadow-emphasize text-left',
  bgCls.value,
  titleCls.value,
  borderCls.value,
  props.class,
...
const statusIcon = computed(() => (props.status !== 'default' ? STATUS_ICON[props.status] : undefined));

const titleTextClass = computed(() => `font-semibold truncate text-text-sm leading-text-sm ${titleCls.value}`);
const descTextClass = computed(() => `font-regular text-text-sm leading-text-xs-2line ${descCls.value}`);
const actionButtonClass = computed(
  () => `group relative w-fit font-semibold text-text-xs leading-text-xs ${actionCls.value} focus-visible:outline-none`,
);
const closeButtonClass = computed(
  () => `group relative flex size-4 items-center justify-center rounded-circle ${closeCls.value} focus-visible:outline-none`,
);
</script>
...
<template>
  <div :class="rootClass">
    <!-- Icon + Content -->
    <div :class="['flex flex-1 min-w-0 items-start', status !== 'default' ? 'gap-1.5' : '']">
      <svg
        v-if="statusIcon"
        width="20"
        height="20"
        ...
        fill="none"
        aria-hidden="true"
        :class="['shrink-0', statusIcon.colorClass]"
      >
        <path
          :fill-rule="statusIcon.fillRule"
          :clip-rule="statusIcon.fillRule ? 'evenodd' : undefined"
          :d="statusIcon.path"
          fill="currentColor"
        />
      </svg>
      ...
      <div class="flex flex-1 min-w-0 flex-col gap-0.5">
        <p :class="titleTextClass">{{ text }}</p>

        <div v-if="hasBody" class="flex flex-col gap-2.5 pb-1">
          <p v-if="showDescription" :class="descTextClass">{{ description }}</p>
          <button v-if="showAction" type="button" :class="actionButtonClass" @click="$emit('action', $event)">
            <span :class="['absolute -inset-0.5 rounded-2', actionOverlay]" />
    ...
    <!-- Close button -->
    <div v-if="showClose" class="flex h-5 shrink-0 items-center">
      <button type="button" aria-label="닫기" :class="closeButtonClass" @click="$emit('close', $event)">
        <span :class="['absolute -inset-1 rounded-circle', closeOverlay]" />
