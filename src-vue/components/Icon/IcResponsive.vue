<!--
  ...
  사용법:
    ...
    <IcResponsive :size="16" />  ← slot 없으면 IcBlank 렌더링
-->
<script lang="ts">
...
import IcBlank from './IcBlank.vue';

withDefaults(defineProps<{ size: IconSize; class?: string }>(), { class: '' });
...
<template>
  <span :class="`inline-flex shrink-0 items-center justify-center ${sizeClass[size]} ${$props.class}`">
    <slot>
      <IcBlank :size="size" />
    </slot>
  </span>
