...
const COLOR_CONFIG: Record<BadgeOption, ColorConfig> = {
  ...
  lightBlue: { text: 'text-light-blue',       border: 'border-light-blue-surface-outline', bg: 'bg-light-blue-container' },
...
export function Badge({
  ...
  const { text, border, bg } = COLOR_CONFIG[option];
  ...
  return (
    <span
      className={[
        ...
        TEXT_CLS[size],
        text,
        type === 'outlined' ? `border ${border}` : bg,
      ].join(' ')}
    >
      {showLeftIndicator && (
        <span
          className={`shrink-0 rounded-full bg-current ${INDICATOR_CLS[size]}`}
          aria-hidden="true"
        />
      )}
