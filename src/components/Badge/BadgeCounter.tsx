/**
 * BadgeCounter (badge_counter)
 *
 * Figma: badge_counter (YqKny45xSmjr76WGIXeL7P)
 *   - node 5919-78872
 *
 * 읽지 않은 수 또는 현재/전체 카운트를 표시하는 배지.
 */

export type BadgeCounterProps = {
  /** 표시할 숫자 */
  count?: number;
  /** 전체 수. 제공 시 "count/total" 형식으로 표시 */
  total?: number;
};

export function BadgeCounter({ count = 1, total }: BadgeCounterProps) {
  return (
    <span
      className={[
        'inline-flex shrink-0 items-center justify-center',
        'bg-secondary-container text-element-brand-variant',
        'rounded-2 px-sol-4 whitespace-nowrap',
      ].join(' ')}
      style={{
        height: 'var(--sol-gap-16)',
        minWidth: 'var(--sol-gap-16)',
