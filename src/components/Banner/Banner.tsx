...
> = {
  ...
  brand:        { bg: 'bg-element-brand-variant',  fg: 'text-element-inverse',  closeFg: 'text-element-inverse-variant', icon: 'responsive' },
  announcement: { bg: 'bg-surface-inverse',         fg: 'text-element-inverse',  closeFg: 'text-element-inverse-variant', icon: 'none' },
};
...
function BannerIcon({ color }: { color: BannerColor }) {
  const { icon } = bannerConfig[color];
...
export default function Banner({
  ...
  const labelId = useId();
  const { bg, fg, closeFg, icon } = bannerConfig[color];
  ...
  const isAlert = color === 'error' || color === 'warning';

  return (
    <aside
      className={`flex w-full items-center justify-between overflow-hidden px-2.5 py-2 text-left ${bg} ${fg} ${className ?? ''}`}
    ...
    >
      ...
      <div className="flex min-w-0 items-center gap-2">
        ...
        <strong
          id={labelId}
          className="truncate text-text-sm font-semibold leading-text-sm"
        >
          {heading}
        </strong>
        {showAction && (
          <button
            type="button"
            className="group relative shrink-0 text-text-sm font-semibold leading-text-sm underline
              focus-visible:outline-none focus-visible:shadow-focus-button-ring"
            onClick={onAction}
          >
            <span className="absolute -inset-0.5 rounded-2 bg-interaction-inverse-normal group-hover:bg-interaction-inverse-hover group-active:bg-interaction-inverse-pressed" />
      ...
      {showClose ? (
        <button
          type="button"
          className={`flex size-4 shrink-0 items-center justify-center rounded-circle ${closeFg}
            focus-visible:outline-none focus-visible:shadow-focus-button-ring`}
